 ...
 */
static void get_toa_data(struct sk_buff *skb, void** sk_user_data_ptr)
{
    struct tcphdr *th;
    int length;
    unsigned char *ptr;
        ...
        while (length > 0) {
            ...
            default:
                ...
                if (opsize > length)
                    ...
                    return ;
                if (( TCPOPT_TOA == opcode || TCPOPT_TOA_TENCENT == opcode ) //加上腾讯lvs这个2B
                            &&  TCPOLEN_TOA == opsize) {
                    TOA_DBG("find toa data: ip = %u.%u.%u.%u, port = %u\n",    NIPQUAD(tdata.ip),ntohs(tdata.port));
                    memcpy(sk_user_data_ptr , ptr - 2, 8);
                    return ;
                }
                ptr += opsize - 2;
                length -= opsize;
        ...
        int *uaddr_len, int peer)
{
    ...
    if (retval == 0 && NULL != sk->sk_user_data && peer) {
        if (sk_data_ready_addr == (unsigned long) sk->sk_data_ready) {
            ...
            if ( ( TCPOPT_TOA == tdata.opcode || TCPOPT_TOA_TENCENT == tdata.opcode ) &&
...
#endif
{
    ...
    if (NULL != newsock && NULL == newsock->sk_user_data) {
        get_toa_data(skb, &newsock->sk_user_data);
